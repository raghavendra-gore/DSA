# Problem: Count Special Integers (Divisible by B and C)

## Problem Description

Scooby has three integers `A`, `B`, and `C`.

Scooby calls a positive integer "special" if it is divisible by `B` **and** it is divisible by `C`. You need to tell the number of special integers less than than or equal to `A`.
